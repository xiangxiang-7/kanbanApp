...
(function (global, factory) {
  ...
  if (
    ...
    var performWorkUntilDeadline = function () {
      ...
      if (scheduledHostCallback !== null) {
        ...
        console.error(
          `--- performWorkUntilDeadline currentTime ${currentTime} ---`,
        );
  ...
  function logEvent(entries) {
    console.log(
      `taskEvent: ${entries[0]}_ms: ${entries[1]}_taskId: ${entries[2]}`,
    );
    if (eventLog !== null) {
  ...
  // 把timeQueue到期的延期任务 push taskQueue任务列表
  function advanceTimers(currentTime) {
    // 把while循环timerQueue列表,把startTime <= currentTime的到期任务放入taskQueue (taskQueue按照expirationTime排序)
    console.error("--- advanceTimers --- ");
  ...
  function handleTimeout(currentTime) {
    console.error("--- handleTimeout 未过期任务调度 ---");
    isHostTimeoutScheduled = false;
    // 把timeQueue到期的任务 push taskQueue任务列表
    advanceTimers(currentTime);

    // 没有执行过期任务调度时 进行过期任务调度
    if (!isHostCallbackScheduled) {
      // 过期任务不为空
      if (peek(taskQueue) !== null) {
        // 执行过期任务调度tag
        isHostCallbackScheduled = true;
        // 执行过期任务调度 传入flushWork
  ...
  // 任务执行
  // 返回值为 workLoop的 true (hasMoreTask--takQueue还有任务没有执行完成 被时间片打断) / false (任务列表全部执行完成)
  function flushWork(hasTimeRemaining, initialTime) {
    console.error(`--- flushWork initialTime ${initialTime}---`);
    {
    ...
    try {
      if (enableProfiling) {
        try {
          // hasTimeRemaining从performWorkUntilDeadine方法传进来一直为 true
          // workLoop返回值为true(视为taskQueue队列没有执行完成) false代表taskQueue为空已经全部执行完成
  ...
  // 循环执行任务  返回true (taskQueue还没执行完 被时间片打断) / false
  function workLoop(hasTimeRemaining, initialTime) {
    console.error("--- workLoop ---");
    var currentTime = initialTime;
    // 先检测一遍timeQueue 往taskQueue里面推任务
    advanceTimers(currentTime);
    // get最先过期的任务
    ...
    while (currentTask !== null && !enableSchedulerDebugging) {
      ...
      if (typeof callback === "function") {
        ...
        // 任务Callback执行返回为function  视为任务没有执行完成
        if (typeof continuationCallback === "function") {
          currentTask.callback = continuationCallback;
          markTaskYield(currentTask, currentTime);
          // 任务Callback执行返回不为function 视为任务已经完成 踢出taskQueue队列
        } else {
          {
    ...
    // 当前taskQueue不为空  任务列表还没执行完成 时间片打断了任务列表while循环
    if (currentTask !== null) {
      return true;
    } else {
      // 当前taskQueue为空 taskQueue已经执行完毕 继续检测timeQueue 为taskQueue输入任务
      var firstTimer = peek(timerQueue);
  ...
  function unstable_scheduleCallback(priorityLevel, callback, options) {
    console.error("--- unstable_scheduleCallback ---");
    var currentTime = getCurrentTime();
    var startTime;
    ...
    if (startTime > currentTime) {
      ...
      if (peek(taskQueue) === null && newTask === peek(timerQueue)) {
        ...
        // 传入延迟时间倒计时 倒计时结束进行未过期任务调度
        requestHostTimeout(handleTimeout, startTime - currentTime);
      }
    } else {
      // 过期任务
      // 过期任务以expirationTime作为最小堆排序
      newTask.sortIndex = expirationTime;
      push(taskQueue, newTask);

      {
        markTaskStart(newTask, currentTime);
        // isQueued 当前任务在过期任务执行队列当中
