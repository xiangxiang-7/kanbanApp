...
(function (global, factory) {
   ...
   */
  var ELEMENT_NODE = 1; // Element节点
  var TEXT_NODE = 3; // Text节点
  var COMMENT_NODE = 8; // #comment 注释文本
  var DOCUMENT_NODE = 9; // Document节点
  var DOCUMENT_FRAGMENT_NODE = 11; // DocumentFragment #document 片段
  ...
  function scheduleCallbackIfUnblocked(queuedEvent, unblocked) {
    console.error("--- scheduleCallbackIfUnblocked ---");
  ...
  function scheduleCallback(reactPriorityLevel, callback, options) {
    console.error(`--- scheduleCallback  开始调度---`);
    // 转换优先级
    var priorityLevel = reactPriorityToSchedulerPriority(reactPriorityLevel);
  ...
  var classComponentUpdater = {
    ...
    enqueueSetState: function (inst, payload, callback) {
      console.error('--- classComponentUpdater enqueueSetState ---')
      var fiber = get(inst);
  ...
  function updateForwardRef(
    ...
    console.error("--- updateForwardRef ---");
  ...
  // 当前组件的tag,根据tag来确认调用的update函数
  function beginWork(current, workInProgress, renderLanes) {
    console.error("--- beginWork ---");
  ...
  function workLoopConcurrent() {
    console.error('--- workLoopConcurrent ---')
  ...
  function commitBeforeMutationEffects() {
    console.error(`--- commitBeforeMutationEffects ---`);
    while (nextEffect !== null) {
  ...
  function FiberRootNode(containerInfo, tag, hydrate) {
    // 标记不同的组件类型  tag为LegacyRoot===0 || BlockingRoot===1 || ConcurrentRoot===2
    this.tag = tag;
    // 挂载的容器dom
  ...
  // 创建FiberRoot
  // containerInfo为容器node
  // tag为LegacyRoot===0 || BlockingRoot===1 || ConcurrentRoot===2
  ...
  // LegacyRoot 模式  tag为LegacyRoot === 0
  // 模式的对比: https://www.jianshu.com/p/34d3fe9d70e0 
  // 模式的对比: https://zh-hans.reactjs.org/docs/concurrent-mode-adoption.html#why-so-many-modes
  ...
  function createRootImpl(container, tag, options) {
    console.error('--- createRootImpl ---', `tag:${tag}`)
  ...
  function createLegacyRoot(container, options) {
    console.error('--- createLegacyRoot --- ')
    return new ReactDOMBlockingRoot(container, LegacyRoot, options);
  }
  // 容器节点是否为元素节点 || document节点 || document片段
...

  {
    ...
    topLevelUpdateWarnings = function (container) {
      // 首次挂载 container._reactRootContaine 为null
      // 不能注释节点
      if (
      ...
      // 首次挂载 isRootRenderedBySomeReact === undified
      var isRootRenderedBySomeReact = !!container._reactRootContainer;
      // 获取节点的都一个子节点
      var rootEl = getReactRootElementInContainer(container);
      // 首次挂载getInstanceFromNode(rootEl) 返回null
      ...
      // 不建议绑定到document.body节点上 因为子节点经常被第三方库操作
      if (
  ...
  // 返回true  子节点存在 && 子节点为元素节点 && 子节点有data-reactroot属性
 ...
 // 创建Root容器节点 {render:()=>{},um}
  ...
  function legacyRenderSubtreeIntoContainer(
    ...
    console.error('--- legacyRenderSubtreeIntoContainer ---')
    {
      // 判断挂载的节点是否符合规范
