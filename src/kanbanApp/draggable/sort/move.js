...
// 排序算法
export default function move({ sourceId, sourceContainerId, sourceContainerList, targetId, targetContainerId, targetContainerList }) {
  ...
  if (sameContainer) {
    return moveWithinContainer(sourceId, targetId, sourceContainerList, targetContainerList);
  // 不同容器 => container splice souce +  push target
  }

  if (differentContainer) {
    return moveOutsideContainer(sourceId, targetId, sourceContainerList, targetContainerList);
  }
    return null;
}
...
function moveOutsideContainer(sourceId, targetId, sourceContainerList, targetContainerList) {
  const sourceIndex = indexOf(sourceContainerList, sourceId)
